...
function notNull(d) {
	return d !== null && typeof d !== 'undefined' && d.trim() !== "";
}
...
module.exports = {
	...
	getSuggestions({
		...
		var args = argData[2];
		var pfx = argData[3];
		if (argIndex === 0 || (args[0] !== "let" && args[0] !== "mod" && args[0] !== "play")) {
			if (!space) {
				...
				switch (argIndex) {
					case 0:
						return this.genResp(
							[this.genSuggestion('frame', 'Creating new Frame', 'keyword'), this.genSuggestion('scene', 'Creating new Scene', 'keyword'), this.genSuggestion('macro', 'Write a macro', 'keyword'), this.genSuggestion('declare', 'Declare a game object', 'keyword'), this.genSuggestion('let', 'Initialize a variable', 'keyword'), this.genSuggestion('mod', 'Modify a variable', 'keyword'), this.genSuggestion('create', 'Create a object', 'keyword'), this.genSuggestion('start', 'Marks the start of the program', 'keyword'), this.genSuggestion('play', 'Plays a specific scene', 'keyword')]);
					case 1:
						switch (args[0]) {
							case "create":
								return this.genResp(
									[this.genSuggestion('character', 'Create a character object', 'type'), this.genSuggestion('stage', 'Create a stage object', 'type'), this.genSuggestion('option', 'Create a option object', 'type'), this.genSuggestion('sound', 'Creates a sound object', 'type')]);
							default:
								return this.genResp([]);
						}
					case 2:
						...
						if (args[0] === "dec" || args[0] === "declare") {
							...
							switch (args[1]) {
								...
								if (isAfterEqual) {
									...
									var method = jointJSON['con'][methodKey];
									...
									var keys = args.join(' ')
										...
										.map(d => d.split(' ').filter(d => d !== null && typeof d === "string" && d.trim() !== "")
											.map(d => d.trim()).pop());
									var method = jointJSON['con'][args[1]];
									var argus = method.arg;
									var ret = [];
									for (var i in argus) {
										var arg = argus[i];
				...
				switch (argIndex) {
					case 0:
						if (pfx.charAt(pfx.length - 1) === ',' || pfx.charAt(pfx.length - 1) === '/') {
							var delimeter = pfx.charAt(pfx.length - 1) === ',' ? ',' : '/';
							var go = pfx.split(delimeter).filter(d => notNull(d)).map(d => d.trim());
							var key = go[0];
							var isit = this.isGameObj(key, o);
							if (isit !== false) {
					...
					case 1:
						var key = args[0].split(/,|\//).filter(d => notNull(d)).map(d => d.trim())[0];
						var isit = this.isGameObj(key, o);
						//console.log(isit);
						if (isit !== false) {
			...
			switch (argIndex) {
				case 1:
					switch (args[0]) {
						case "play":
							return this.genResp([this.genSuggestion('scene id', 'string', 'suggestion')]);
					}
					return this.genResp([]);
				case 2:
					switch (args[0]) {
						...
						case "play":
							return this.genResp(
								[this.genSuggestion('frameID', 'number', 'sugggestion')]
							);
						default:
							return this.genResp([]);
					}
	...
	getGameObjects(editor) {
		...
		var lines = text.split('\n')
			.filter(d => d !== null && typeof d === "string" && d.trim() !== "")
			.map(d => d.trim())
			.filter(d => d.split(' ')[0].trim() === "declare" || d.split(' ')[0].trim() === "dec" || d.split(' ')[0].trim() === "create")
		var objects = {};
		for (var i in lines) {
			var line = lines[i];
			var args = line.split(' ')
			...
			if (args.length === 4) {
				...
				if (key === "declare" || key === "dec") {
					if (as === "as") {
						if (type === "stage" || type === "character" || type === "option" || type === "sound") {
							var ins = chars.split(',')
								.filter(d => d !== null && typeof d === "string" && d.trim() !== "")
								.map(d => d.trim())
							for (var x in ins) {
